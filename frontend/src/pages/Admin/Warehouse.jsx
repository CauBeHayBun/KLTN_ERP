...
import { 
  ...
  Calendar, ChevronRight, CheckCircle2, Clock, FileText, Zap, User, Phone
} from 'lucide-react';
          ...
          <div style={{
            ...
            <div style={{ padding: '1rem', backgroundColor: '#f0fdf4', border: '1px solid #bbf7d0', borderRadius: '12px', marginBottom: '1.25rem' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <span style={{ color: '#0f172a', fontWeight: 700, fontSize: '1rem' }}>Tổng Giá Trị Đơn Hàng:</span>
                <span style={{ color: '#16a34a', fontSize: '1.35rem', fontWeight: 800 }}>{formatPrice(selectedDetailOrder.totalAmount)}</span>
              </div>
            ...
            <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '0.75rem' }}>
              {selectedDetailOrder.status === 'CONFIRMED' && (
                <button
                  onClick={() => {
                    if (updateOrderStatus) {
                      updateOrderStatus(selectedDetailOrder.orderId, 'READY_TO_SHIP', 'Đã xuất kho – Chờ bên giao hàng nhận đơn');
                      setSelectedDetailOrder(prev => ({ ...prev, status: 'READY_TO_SHIP' }));
                      alert(`✅ Đã xác nhận xuất kho đơn hàng ${selectedDetailOrder.orderId}!`);
                    }
                  }}
                  className="btn btn-primary"

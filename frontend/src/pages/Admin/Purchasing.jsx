...
export default function Purchasing() {
  ...
  useEffect(() => {
    ...
    window.addEventListener('set-purchasing-status-filter', handleSetFilter);
    return () => window.removeEventListener('set-purchasing-status-filter', handleSetFilter);
  }, []);
            ...
            <table className="erp-table" style={{ width: '100%', borderCollapse: 'collapse' }}>
              ...
              <tbody>
                {filteredOrders.map((po) => (
                  <tr key={po.id} onClick={() => { setSelectedPO(po); setViewMode('PO'); }} style={{ cursor: 'pointer', transition: 'background 0.2s' }} className="hover-row">
                    <td style={{ padding: '0.7rem 1rem', fontWeight: 700, color: '#818cf8', textAlign: 'left', fontSize: '0.85rem' }}>{po.poNumber}</td>
                    <td style={{ padding: '0.7rem 1rem', fontWeight: 600, textAlign: 'left', fontSize: '0.85rem' }}>{po.supplier?.name || po.supplierCode}</td>
                    <td style={{ padding: '0.7rem 1rem', color: 'var(--text-secondary)', textAlign: 'left', fontSize: '0.82rem' }}>{po.createdBy || 'N/A'}</td>
                    <td style={{ padding: '0.7rem 1rem', color: 'var(--text-secondary)', textAlign: 'left', fontSize: '0.82rem' }}>{formatDate(po.expectedDeliveryDate)}</td>
                    <td style={{ padding: '0.7rem 1rem', fontWeight: 700, textAlign: 'left', fontSize: '0.88rem' }}>{formatCurrency(po.totalAmount)}</td>
      ...
      {showCompareModal && (
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: 'rgba(15,23,42,0.6)', backdropFilter: 'blur(8px)', display: 'flex', justifyContent: 'center', alignItems: 'center', zIndex: 1050, padding: '1.5rem' }}>
          <div className="card-glass" style={{ width: '100%', maxWidth: '1200px', maxHeight: '92vh', overflowY: 'auto', padding: '2rem', backgroundColor: '#ffffff', borderRadius: '24px', border: '1px solid #cbd5e1', boxShadow: '0 30px 60px rgba(0,0,0,0.15)' }}>
            ...
            {/* Fetch 3 RFQs for Comparison */}
            {(() => {
              // Group active RFQs or get up to 3 candidate RFQs
              const candidatePos = orders.filter(po => ['RFQ', 'RFQ_SENT', 'QUOTED'].includes(po.status));
              const displayPos = candidatePos.length >= 3 ? candidatePos.slice(0, 3) : orders.slice(0, 3);

              if (displayPos.length === 0) {
                return (
                  <div style={{ textAlign: 'center', padding: '4rem', color: '#64748b' }}>
                    <AlertCircle size={48} style={{ margin: '0 auto 1rem', opacity: 0.4 }} />
                    <p style={{ fontSize: '1.1rem', fontWeight: 600 }}>Chưa có đơn Yêu Cầu Báo Giá (RFQ) nào để so sánh.</p>
                    <button onClick={() => { setShowCompareModal(false); setShowCreateModal(true); }} className="btn btn-primary" style={{ marginTop: '1rem', borderRadius: '10px' }}>
                      <Plus size={16} /> Tạo YCBG Cho 3 NCC Ngay
                    </button>
                  </div>
                );
              }

              // Compute Min Price and Max Price for savings calculation
              ...
              return (
                <div>
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', gap: '0.85rem' }}>
                        ...
                        <div>
                          ...
                          <span style={{ fontSize: '0.82rem', color: '#047857' }}>
                            Chọn phương án báo giá rẻ nhất giúp doanh nghiệp tiết kiệm <strong style={{ color: '#059669', fontSize: '0.95rem' }}>{formatCurrency(savingsAmount)} ({savingsPercent}%)</strong> so với báo giá cao nhất!
                          </span>
                        </div>
                      </div>
                  ...
                  {/* 3 Supplier Comparison Cards Matrix */}
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '1.25rem', marginBottom: '2rem', alignItems: 'stretch' }}>
                    {displayPos.map((po, idx) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <div>
                            {/* Supplier Header with fixed min-height for perfect alignment */}
                  ...
                  <div style={{ background: '#f8fafc', border: '1px solid #e2e8f0', borderRadius: '16px', padding: '1.25rem' }}>
                    ...
                    <div className="table-container" style={{ borderRadius: '10px', overflow: 'hidden' }}>
                      <table className="erp-table">
                        ...
                        <tbody>
                          {/* Extract items list from first PO or all */}
